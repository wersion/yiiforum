...
class ThreadController extends BaseFrontController {
	...
	public function actionIndex() {
		...
		$query = Thread::find ()->where ( [ 
				'board_id' => $boardId,'status' => 1 
		] );
	...
	public function actionView($id) {
		...
		$locals = YiiForum::getPagedRows ( $query, [ 
				'order' => 'create_time asc','pageSize' => 10 
		] );
	...
	public function actionNewThread() {
		...
		if ($model->load ( Yii::$app->request->post () )) {
			...
			return $this->redirect ( [ 
					'view','id' => $model->id,'boardid' => $boardId 
			] );
		} else {
			$locals = [ ];
			$locals ['model'] = $model;
